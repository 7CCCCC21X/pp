...
| `/combo` | Adjacent-rung combo screen: on FDV/market-cap ladders buy the **lower rung's YES + next rung's NO**; on launch-date ladders (`2026年9月30日` / `Sep 30, 2026` buckets) buy the **earlier date's NO + next date's YES**. Both legs together settle ≥ $1, and $2 when the outcome lands between the two thresholds, so any pair whose combined cost is under the cap (default **110¢**) is listed (under 100¢ = pure arb 🔥). Filter first, query second: `/combo` opens the wizard card (same interaction as `/tight`) — cost cap × ladder type (all/money/date) × minimum executable shares × 极端价 filter (exclude **or** show-only pairs where either leg is near-decided, e.g. `排除 ≥85¢`, judged on the freshly refetched books) × **时间误差** filter (date ladders only, measured on the hedge combo itself — **buy the earlier date's NO + the later date's YES**: the earlier rung's fair price ≈ the later rung's mid × remaining-time ratio — e.g. when Sep 30 is half as far away as Dec 31 it "should" trade near half the Dec 31 price — so the combo's fair total = 100 + later mid × (1 − ratio); turning the knob on lists adjacent date pairs whose mid-based combo cost deviates from that fair total by ≥N¢ **instead of** gating by the cost cap, so badly mispriced pairs are viewable even when their combined cost is way above it. Every date pair renders a `⏱ 时间比价` line with the combo's mid cost, its time-fair value and the signed 误差 — positive = combo rich, the reverse legs (earlier YES + later NO) are the better side; negative = the listed combo is the bargain) × sort (cost / shares / middle-band $ / hedgeable amount — depth-aware volume matchable across both legs' book levels while the combined price stays under the cap / stall time — how long both legs' books have sat unmoved / 时间误差 — largest deviation first), with `✏` typed-custom input for the cap, share, 极端价 and 时间误差 knobs. `🚀` re-fetches every rung's orderbook live — never the tick cache — and renders the results **paginated** (⬅️/➡️ page through the cached scan; hit `🚀` again for fresh prices). `/combo 105` pre-sets the cap, `/combo set 108` persists it. `/combo check <URL\|id>` (or just pasting a market URL after `/combo`) runs a step-by-step diagnosis of why a market is or isn't picked up: monitoring status, threshold parsing, ladder grouping (with each rung's exclusion reason), and live adjacent-pair costs vs the cap |
...
| `/timeerr` | Quick-menu shortcut into the `/combo` wizard pre-set for the date time-error screen (📅 date ladders · 时间误差 ≥2¢ · sorted by deviation) — one `🚀` tap away from the scan. `/timeerr 1` tunes the minimum error. Also reachable from `/menu` via the `⏱ 时间误差` button |
