...
| `/combo` | Adjacent-rung combo screen: on FDV/market-cap ladders buy the **lower rung's YES + next rung's NO**; on launch-date ladders (`2026年9月30日` / `Sep 30, 2026` buckets) buy the **earlier date's NO + next date's YES**. Both legs together settle ≥ $1, and $2 when the outcome lands between the two thresholds, so any pair whose combined cost is under the cap (default **110¢**) is listed (under 100¢ = pure arb 🔥). Re-fetches every rung's orderbook live on each run — never the tick cache. `/combo 105` one-off cap, `/combo set 108` persists. The result's bottom `🎚` button opens a wizard (same interaction as `/tight`): cost cap × ladder type (all/money/date) × minimum executable shares × 极端价 filter (exclude **or** show-only pairs where either leg is near-decided, e.g. `排除 ≥85¢`, judged on the freshly refetched books) × sort (cost / shares / middle-band $ / stall time — how long both legs' books have sat unmoved), with `✏` typed-custom input for the cap, share, and 极端价 knobs. All hits are listed (long results auto-split across messages) |
